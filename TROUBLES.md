...
### Перевірено (3 тести, усі пройшли)
- "так, закомміть і запуште" → KEEP (новий виняток)
- "перевір усе" → DELEGATE (без змін, як і мало бути)
- "помилка в коді" → KEEP (старий виняток за ключовим словом,
  без змін)
